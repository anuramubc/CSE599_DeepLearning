...
def discriminator(seed=None):
    ...
    model = nn.Sequential(
      #First call flatten()
      Flatten(),
      #First FC with size (784,256)
      nn.Linear(784,256),
      #First LeakyReLU with alpha = 0.01
      nn.LeakyReLU(0.01),
      #Second FC with (256,256)
      nn.Linear(256,256),
      #Second LeakyReLU with alpha = 0.01
      nn.LeakyReLU(0.01),
      #Last FC with size(256,1)
      nn.Linear(256,1)
    )
...
def generator(noise_dim=NOISE_DIM, seed=None):
    ...
    model = nn.Sequential(
      #First FC with size (moise_dim,1024)
      nn.Linear(noise_dim, 1024),
      #First ReLU 
      nn.ReLU(),
      #Second FC with (1024,1024)
      nn.Linear(1024,1024),
      #Second ReLU
      nn.ReLU(),
      #Last FC with size(1024,784)
      nn.Linear(1024,784),
      #Implement tanh to clip the image to be in range [-1,1]
      nn.Tanh()
    )
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    #real_Scores = 0.5*E[(D(x)-1)^2]
    #fake_Scores = 0.5*E[(D(G(x)))^2]
    #loss = real_Scores + fake_Scores
    real_scores = 0.5 * torch.mean((scores_real - 1)**2)
    fake_scores = 0.5 * torch.mean((scores_fake)**2)
    loss = real_scores + fake_scores
