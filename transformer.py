...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        #Step 1: Embed captions using self.embedding
        embedded_caption = self.embedding(captions)
        #Step 2: add positional encoding using self.positional_encoding
        added_pos_embed = self.positional_encoding(embedded_caption)
        #Step 3: Project the image features into same dimensions using self.visual_projection
        projected_img_features = self.visual_projection(features).unsqueeze(1)
        #Step 4: mask variable to mask out the features
        tgt_mask = torch.ones((T,T), dtype = bool)
        tgt_mask = torch.tril(tgt_mask) 
        #Step 5: Apply decoder on embeddings with tgt_mask
        decoder_output = self.transformer(tgt = added_pos_embed, memory = projected_img_features, tgt_mask = tgt_mask)
        #Step 6: Project the decoder output per token to get scores
