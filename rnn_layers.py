...
def rnn_step_backward(dnext_h, cache):
    ...
    #derivative of tanh^2x = sech^2x = 1- tanh^2x
    #db = [1- tanh^2(Whh h_t-1 + Wht xt + b)] dnext_h
    #dWhh = [1- tanh^2(Whh h_t-1 + Wht xt + b)] dnext_h * h_t
    #dWht = [1- tanh^2(Whh h_t-1 + Wht xt + b)] dnext_h * xt
    #dht = [1- tanh^2(Whh h_t-1 + Wht xt + b)] dnext_h * Whh
    #dxt = [1- tanh^2(Whh h_t-1 + Wht xt + b)] dnext_h * Wht
    #Here dnext_h is the upstream gradient
    #Let dz = [1- tanh^2(Whh h_t-1 + Wht xt + b)] dnext_h 
    dz = np.multiply((1-next_h**2), dnext_h)
    db = np.sum(dz,axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    #Call rnn_step_forward for each timestamp and store the current values in h and previous values in cache
    cache = []
    N,T,D = x.shape
    H = h0.shape[1]

    #Create empy array of all hidden states of shape (N, T, H)
    h = np.zeros((N,T,H))
    #initially let h_t = h0
    prev_h = h0
    #looping through all time steps T of imput data and calling rnn_step_forward using data at that timestep, 
    #cuurent_h, Wx,Wh, b
    for t in range(T):
      h[:,t,:], next_cache =  rnn_step_forward(x[:,t,:], prev_h, Wx,Wh, b)
      #update the prev_h to be new h
      prev_h = h[:,t,:]
...
def rnn_backward(dh, cache):
    ...
    #Initialize dx, dh0, dWx, dWh, db to respective shapes
    db = np.zeros(H)
    dx = np.zeros((N,T,D))
    dh0 = np.zeros((N,H))
    dWx = np.zeros((D,H))
    ...
    # For back prop start from the last timestep and decrement it by 1 and call rnn_step_backward()
    for t in range(T-1, -1,-1):
      #During each time step, the dprev_h will possess all gradients from the last timestep to current
      dprev_h += dh[:,t,:]
      #call rnn_step_backward() with the upstream gradient = dprev_h and cache = cache[t]
      d_x, dprev_h, d_Wx, d_Wh, d_b = rnn_step_backward(dprev_h, cache[t])
      dx[:,t,:] = d_x
      dWx += d_Wx
      dWh += d_Wh
      db  += d_b
...
def word_embedding_forward(x, W):
    ...
    #return the W rows corresponding to x values
...
def word_embedding_backward(dout, cache):
    ...
    W,x = cache
    #Shape of dW = (V,D) = shape of W
    dW = np.zeros(W.shape)
    #np.add.at(a, indices,b) : add b to a at indices
    np.add.at(dW, x, dout)
